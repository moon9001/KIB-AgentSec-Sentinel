...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "scoring": {
        "label_threshold": 45,
        "max_score": 100,
        "risk_levels": {"low": 15, "medium": 35, "high": 55, "critical": 75},
    },
    "llm": {
        "enabled": False,
    ...
    "output": {"evidence_excerpt_chars": 240},
}
...
DEFAULT_RULES: dict[str, Any] = {
    "weights": {
        ...
        "compression_archive": 12,
        "network_exfil": 18,
        "destructive_command": 60,
        "privilege_escalation": 20,
        "trace_cleanup": 28,
        "agent_tool_abuse": 20,
        "suspicious_network": 14,
        "combo_sensitive_archive_network": 45,
        "combo_cleanup_plus_sensitive": 35,
        "combo_privilege_followup": 35,
        "combo_credential_exfil": 50,
        "correlated_agent_audit_network": 30,
    },
    "keywords": {
        "sensitive_paths": ["/etc/passwd", "/etc/shadow", ".ssh/", "id_rsa", "authorized_keys"],
        "credential_keywords": ["token", "secret", "credential", "passwd", "shadow", "id_rsa", "api_key"],
        "read_markers": ["read_file", "cat", "open", "copy", "cp", "download"],
        "compression_tools": ["tar", "zip", "gzip", "7z", "rar", "archive"],
